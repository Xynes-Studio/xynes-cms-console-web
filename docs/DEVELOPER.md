...
### Favourites chip filter (BUG-CMS-11, landed 2026-06-01)

The CMS content-list toolbar carries a "Favourites" chip (`onFavoritesToggle` in `CmsContentToolbar.tsx`) that writes `favoritesOnly` to the URL-backed query state via `useCmsContentQueryState`. Before BUG-CMS-11, toggling the chip flipped the URL flag and re-rendered the toolbar in its active state, but the entries list itself never honoured the filter — `visibleItems` in `CmsContentListPanel.tsx` only filtered out deleted entries and applied `favoriteOverrides`. Non-favourite entries continued to render.

**Fix.** `visibleItems` now runs the favourites filter AFTER the `favoriteOverrides` merge in `CmsContentListPanel.tsx`. The ordering matters: applying the filter after the override means an optimistic un-favourite hides the row in the same render cycle, and a rollback (server rejected the toggle) flips `favoriteOverrides[id]` back to `true` and re-includes the row immediately — without a refetch.

**Empty-state copy.** `CmsContentListState.tsx` `resolveCmsContentListState` accepts an optional `favoritesOnly` flag and a `favoritesEmptyTitle` / `favoritesEmptyDescription` copy pair. When `favoritesOnly === true` and the resolved count is zero, the favourites-empty branch wins over the directory/root-empty branch so the user understands the filter is the cause. New en-US keys: `state.favoritesEmptyTitle = "No favourites yet"` and `state.favoritesEmptyDescription = "Star entries you want to revisit, or turn off the favourites filter to see everything."` The en-XA pseudo-locale carries deterministic ASCII-doubled equivalents (same approach as BUG-CMS-7 / BUG-CMS-10). 14/14 leaf-key parity between en-US and en-XA verified.

**Tests landed.** Five new BUG-CMS-11 tests in `CmsContentListPanel.test.tsx` cover: (a) all-entries baseline (chip off), (b) favourites-only renders the filtered subset and propagates the count, (c) optimistic toggle-off hides the row immediately under favourites-on, (d) rollback restores the row on server rejection + surfaces the existing destructive toast, (e) favourites-empty copy when the chip is on and no entry is favourited (asserting the directory-empty copy is NOT shown). Two additional tests in `CmsContentListState.test.ts` lock the resolver's favourites-empty branch. Lint clean; `npx tsc --noEmit` 17 errors byte-identical to `main` (zero new TS errors — only the line numbers of the two pre-existing `mockImplementationOnce` typing complaints shifted by +3 because the next-intl mock grew by 3 lines); full suite **653/653 pass / 60 files** (+7 net); coverage `CmsContentListPanel.tsx` **96.82% / 88.59% / 86.66% / 96.82%** and `CmsContentListState.tsx` **100% / 100% / 100% / 100%** (both above ADR-001 80% floor); overall **93.97% / 87.7% / 96.39% / 93.97%**; `pnpm build` ✓ Compiled successfully. Visual review deferred to operator per sprint-plan §8 manual-smoke-accepted posture.

**Regression-guard validation.** Temporarily stashing `CmsContentListPanel.tsx` (the panel fix alone) while keeping every test, catalog, and resolver change in place causes **3 of the 5 new tests to fail** deterministically — confirming the new tests would catch a regression. Specifically: the favourites-on render test, the optimistic-toggle-hide test, and the favourites-empty-copy test all fail. The "renders every entry when chip off" test and the "rollback restores the row" test stay green under both states by design — the first locks the no-regression-when-chip-off baseline; the second locks the rollback contract irrespective of which side of the filter the row ends up on.

**Contracts preserved.** The optimistic toggle flow from the existing `list-card delete and toggle-favorite` describe block is byte-for-byte unchanged: `pendingFavoriteIds` still throttles, `favoriteOverrides` still flips, the `"Could not update favourite"` toast still surfaces on rejection. The `useCmsContentEntries` hook is unchanged — the favourites filter remains client-side because the entry list is already workspace-scoped and server-paged, and an upstream `favoritesOnly` server filter would (a) require coordinating with the cms-core action handler and (b) not interact correctly with the optimistic-override layer (a row optimistically un-favourited would still be in the server's response until the request settled). Client-side filtering after override merge is the right boundary for this UX.
